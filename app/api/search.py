...
def build_ask_prompt(question: str, context: str) -> str:
    return (
        ...
        "If the user asks for a total count, aggregate statistic, or metadata summary across all programs, "
        "do not guess from a few retrieved chunks. Instead say that the retrieved context is not sufficient "
        "to determine the full count or full-dataset answer. "
        "Do not infer totals, percentages, or complete coverage from partial examples. "
        ...
        "3. If the question asks for a count or full-dataset summary and the context is partial, "
        "explicitly say the answer cannot be determined from the retrieved context.\n"
        "4. Do not mention information that is not present in the context.\n\n"
        f"Question:\n{question}\n\n"
        f"Context:\n{context}\n"
    )
        ...
        prompt_template = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    "If the context is insufficient to answer fully, say so explicitly. "
                    "If the user asks for a total count, aggregate statistic, or metadata summary across all programs, "
                    "do not guess from a few retrieved chunks. Say the retrieved context is insufficient for a full count "
                    "or full-dataset answer. "
                    "Do not infer totals, percentages, or complete coverage from partial examples. "
                    "Keep the answer concise, factual, and grounded in the retrieved content. "
